...
function nextSlide(){
    ...
    // Función para mostrar o no la sección #lead (MAIN)
...
// Función para mostrar color verde sobre secciones del menú lateral cuando nos desplazamos en la web
document.addEventListener("DOMContentLoaded", function () {
    const menuLinks = document.querySelectorAll('#menu a');
    
    // Función para iluminar el enlace del menú correspondiente
    function highlightMenuLink(sectionId) {
        menuLinks.forEach(link => {
            link.classList.remove('highlighted'); // Remueve la clase de todos los enlaces
            if (link.getAttribute('href') === `#${sectionId}`) {
                link.classList.add('highlighted'); // Añade la clase al enlace correspondiente
            }
        });
    }

    // Añade el evento de desplazamiento para detectar en qué sección se encuentra el usuario
    window.addEventListener('scroll', () => {

        let currentSection = '';

        document.querySelectorAll('.sectionmenu').forEach(div => {
        ...
        if (currentSection) {
            highlightMenuLink(currentSection);
        }
